...
```{R}
dat.f <- read.csv("~/Dropbox/Crane_trachea/data/Crane_Trachea.csv", header =TRUE)

dat.morpho <- read.csv("~/Dropbox/Crane_trachea/crane_morpho.csv", header =TRUE)
...
dat.f$mean_trachea_length <-rowMeans(subset(dat.f, select = c(length_mm_1, length_mm_2, length_mm_3)), na.rm = TRUE) #make mean for three length measurements a variable, then add in mean trachea lengths from older subset whihc are maybe means already? FIND OUT!

for(i in 1:nrow(dat.f)){
  if(is.na(dat.f$mean_trachea_length[i]))
    dat.f$mean_trachea_length <-dat.f$length_mm_1[i]
}

dat.f$width._1st_curve <-as.numeric(dat.f$width._1st_curve) #NOTE making character value of trachea width to a numerical value
dat.f$segment_density <- dat.f$segment_count2/dat.f$mean_trachea_length #NOTE create segment density

dat.f <- dat.f %>%
  mutate_if(sapply(dat.f, is.character), as.factor)%>%
mutate(mass = as.numeric(as.character(mass)),
  log_mass = log10(mass),
       log_mass13 = log10(mass^(1/3)),
  prop_trachea = mean_trachea_length/mass,
            log_mtl = log10(mean_trachea_length),
            uncertain_sex = factor(if_else(is.na(uncertain_sex), 0,1)))
...
#make dummy variable to allow to easily drop suspect cranes
dat.f <- dat.f%>%
  mutate(sus = ifelse(dat.f$taxon_name == "Antigone canadensis tabida" & dat.f$sex =="M" & dat.f$mass < 4800 | dat.f$taxon_name == "Antigone canadensis canadensis" & dat.f$mass < 2400, "sus", "chill"))%>%
  left_join(., dat.morpho, by = "NK")%>%
  mutate( BMI.tars = (mass/(tarsus)^2)*105,
          BMI.trach =(mass/(mean_trachea_length^2)*105),
          BMI.wing = (mass/(wing_chord^2)*105))


r <- dat.f %>%
  filter(!is.na(log_mtl))%>%
  group_by(taxon_name, sex) %>%
  do(model = lm(log_mtl ~ log_mass13, data=dat.f)) %>%
  do((function(mod) {
...
r <- dat.f %>%
  filter(!is.na(log_mtl))%>%
  group_by(taxon_name, sex) %>%
  do(augment(lm(log_mtl ~ log_mass13, data=.)))
...
dat.s <- dat.f%>% filter(dat.f$sus == "chill")
dat.s <- dat.s%>% filter(dat.s$sex %in% c("M", "F"))
dat.baby <-dat.s%>% filter(dat.s$bursa == "Y")
dat.s.conservative <- dat.s %>%
  filter(!(taxon_name=="Antigone canadensis canadensis" & mass <3250  & sex=="M"),
         !(taxon_name=="Antigone canadensis canadensis" & mean_trachea_length ==625  & sex=="F"),
         !(taxon_name=="Antigone canadensis canadensis" & mass ==625  & sex=="F")) #after exploration below uncertain sex doesn't help much but removes some uncertainty. for lessers most uncertain points fall within "cloud" of same sex. However bursa shows several very small males  (mass <3250) falling out with females. Removing these

#remove female greaters less than 4000 just one remove by NK and lesser over 4500
dat.s <- dat.s%>%
  filter(!NK %in% c(281809, 283503))
...
dat.g$log_mass13.z <- standardize(dat.g$log_mass13)
dat.g$segment_count1.z <- standardize(dat.g$segment_count1)
dat.g$segment_density.z <- standardize(dat.g$segment_density)
dat.g$prop_trachea.z <- standardize(dat.g$prop_trachea)


#First scale some predictors.
dat.l$log_mass.z <- standardize(dat.l$log_mass)
dat.l$log_mass13.z <- standardize(dat.l$log_mass13)
dat.l$segment_count1.z <- standardize(dat.l$segment_count1)
dat.l$segment_density.z <- standardize(dat.l$segment_density)
dat.l$prop_trachea.z <- standardize(dat.l$prop_trachea)
#SUBSET BY SEX AND TAXON
...
#add Vonbank et al. assignment column from their algorithm
dat.s<-dat.s%>%
  left_join(., Probabilities, by = "NK")
```
...
plotNormalHistogram(log10(dat.l$mean_trachea_length)) #looks decent
...
```{r, echo=F, figures-side, fig.show="hold", out.width="50%"}
plotNormalHistogram(dat.g$log_mass13) #looks great
plotNormalHistogram(dat.l$log_mass13) #looks not so decent but we expect some separation because of the two groups #remove that small male
...
```{r, echo=F, figures-side, fig.show="hold", out.width="50%"}
qqPlot(dat.g$log_mass13)
qqPlot(dat.l$log_mass13) #hmmm 24 & 48 look like they may need an eye

```

```{r}
ggplot(dat.l%>%filter(taxon_name=="Antigone canadensis canadensis"), aes(mass, mean_trachea_length, color=uncertain_sex, shape=sex))+
  geom_point()

```

```{r}
ggplot(dat.s.conservative%>%filter(taxon_name=="Antigone canadensis tabida"), aes(mass, mean_trachea_length, color=uncertain_sex, shape=sex))+
  geom_point()
...
dat.f %>%
  ...
  summarise(count=n(),
            ...
            mean.segment = mean(segment_count2, na.rm=T),
            sd.segment = sd(segment_count2, na.rm=T))

```
### Conservative dataset
```{r}
#by subspecies & sex
t<-dat.s.conservative %>%
  group_by(taxon_name, sex)%>%
...
p<-dat.s %>%
  ...
  summarise(count=n(),
            ...
            mean.mass = mean(mass, na.rm=T),
            sd.mass = sd(mass, na.rm=T))%>%
  mutate(se.tl = sd.mean.trachea.length / sqrt(count), #95% CI
         lower.ci.tl = mean.trachea.length - qt(1 - (0.05 / 2), count - 1) * se.tl,
         upper.ci.tl = mean.trachea.length + qt(1 - (0.05 / 2), count - 1) * se.tl)%>%
  ggplot(., aes(taxon_name, mean.trachea.length))+
  geom_pointrange(mapping = aes(ymin = lower.ci.tl, ymax = upper.ci.tl))+
  geom_point(size=4, shape=21, fill="grey65")+
  theme_classic(base_size = 14)+
  scale_x_discrete(labels=c("A. c. canadensis", "A. c. tabida"))+
  theme(axis.text.x = element_text(face = "italic"))+
  labs(x="", y= "mean trachea length (mm)")
p
ggsave(p, filename = paste(getwd(), "/figures/Mean_trachea_length_subspecies_pointrange.pdf", sep=""),  bg = "transparent", height = 8, width = 5, units = "in")
...
p<-dat.s %>%
  group_by(taxon_name)%>%
  ggplot(., aes(taxon_name, mean_trachea_length))+
  geom_violin(trim=F)+
  ...
  labs(x="", y= "mean trachea length (mm)")
p
  ggsave(p, filename = paste(getwd(), "/figures/Mean_trachea_length_subspecies_violin.pdf", sep=""),  bg = "transparent", height = 8, width = 5, units = "in")
...
p<-dat.s.conservative %>%
  ...
  mutate(se.tl = sd.mean.trachea.length / sqrt(count), #95% CI
         lower.ci.tl = mean.trachea.length - qt(1 - (0.05 / 2), count - 1) * se.tl,
         upper.ci.tl = mean.trachea.length + qt(1 - (0.05 / 2), count - 1) * se.tl)%>%
  ggplot(., aes(taxon_name, mean.trachea.length, fill=sex))+
  geom_pointrange(mapping = aes(ymin = lower.ci.tl, ymax = upper.ci.tl, group=sex, fill=sex))+
  geom_point(size=4, shape=21)+
  scale_fill_manual(values=c("darkseagreen2", "burlywood2"))+
  theme_classic(base_size = 14)+
  scale_x_discrete(labels=c("A. c. canadensis", "A. c. tabida"))+
  theme(axis.text.x = element_text(face = "italic"),
  ...
  labs(x="", y= "mean trachea length (mm)")
p
ggsave(p, filename = paste(getwd(), "/figures/Mean_trachea_length_subspecies_sex_pointrange.pdf", sep=""),  bg = "transparent", height = 8, width = 5, units = "in")
...
p<-dat.s %>%
  ...
  labs(x="", y= "mean trachea length (mm)")
p
  ggsave(p, filename = paste(getwd(), "/figures/Mean_trachea_length_subspecies_sex_violin.pdf", sep=""),  bg = "transparent", height = 4, width = 4, units = "in")
...
p<-dat.s.conservative %>%
  ...
  summarise(count=n(),
            ...
            mean.mass = mean(mass, na.rm=T),
            sd.mass = sd(mass, na.rm=T))%>%
  mutate(se.m = sd.mass / sqrt(count),
         lower.ci.m = mean.mass - qt(1 - (0.05 / 2), count - 1) * se.m,
         upper.ci.m = mean.mass + qt(1 - (0.05 / 2), count - 1) * se.m)%>%
  ggplot(., aes(taxon_name, mean.mass))+
  geom_pointrange(mapping = aes(ymin = lower.ci.m, ymax = upper.ci.m))+
  geom_point(size=4, shape=21, fill="grey65")+
  theme_classic(base_size = 14)+
  scale_x_discrete(labels=c("A. c. canadensis", "A. c. tabida"))+
  theme(axis.text.x = element_text(face = "italic"))+
  labs(x="", y= "mean mass (g)")
p
ggsave(p, filename = paste(getwd(), "/figures/Mass_subspecies_violin.pdf", sep=""),  bg = "transparent", height = 8, width = 5, units = "in")
...
p<-dat.s %>%
  ...
  labs(x="", y= "mass (g)")
p
  ggsave(p, filename = paste(getwd(), "/figures/Mean_mass_subspecies_sex_violin.pdf", sep=""),  bg = "transparent", height = 4, width = 4, units = "in")
...
p<-dat.s %>%
  ...
  labs(x="", y= "BMI (tarsus)")
p
  ggsave(p, filename = paste(getwd(), "/figures/BMI_tars_subspecies_sex_violin.pdf", sep=""),  bg = "transparent", height = 4, width = 4, units = "in")
...
p
  ggsave(p, filename = paste(getwd(), "/figures/BMI_trach_subspecies_sex_violin.pdf", sep=""),  bg = "transparent", height = 4, width = 4, units = "in")
...
p
  ggsave(p, filename = paste(getwd(), "/figures/BMI_wing_subspecies_sex_violin.pdf", sep=""),  bg = "transparent", height = 4, width = 4, units = "in")
...
p<-dat.s %>%
  ...
  filter(!bursa %in% c("yes"),
         mass >= 2400)%>%
  ggplot(., aes(taxon_name, resid, fill=sex))+
  geom_violin(trim=F)+
  geom_boxplot(width=0.1,  position = position_dodge(width = 0.9))+
  theme_classic(base_size = 14)+
  scale_x_discrete(labels=c("A. c. canadensis", "A. c. tabida"))+
  ...
  theme(axis.text.x = element_text(face = "italic"),
        legend.position = c(0.2,0.85))+
  labs(x="", y= "Residuals(log(mass ~ mtl))")
p
  ggsave(p, filename = paste(getwd(), "/figures/Residual_subspecies_sex_violin.pdf", sep=""),  bg = "transparent", height = 4, width = 4, units = "in")
...
p<-dat.s %>%
  ...
  labs(x="", y= "tarsus (mm)")
p
  ggsave(p, filename = paste(getwd(), "/figures/Tarsus_subspecies_sex_violin.pdf", sep=""),  bg = "transparent", height = 4, width = 4, units = "in")
...
p
  ggsave(p, filename = paste(getwd(), "/figures/Wing_chord_subspecies_sex_violin.pdf", sep=""),  bg = "transparent", height = 4, width = 4, units = "in")
...
p
  ggsave(p, filename = paste(getwd(), "/figures/Anterior_culmen_subspecies_sex_violin.pdf", sep=""),  bg = "transparent", height = 4, width = 4, units = "in")
...
p
  ggsave(p, filename = paste(getwd(), "/figures/Proportion_trachea_subspecies_sex_violin.pdf", sep=""),  bg = "transparent", height = 4, width = 4, units = "in")
...
p
  ggsave(p, filename = paste(getwd(), "/figures/Segment_count_subspecies_sex_violin.pdf", sep=""),  bg = "transparent", height = 4, width = 4, units = "in")
...
p<-dat.s %>%
  ...
  filter(!bursa %in% c("yes"),
         mass >= 2400)%>%
  ggplot(., aes(taxon_name, segment_density, fill=sex))+
  geom_violin(trim=F)+
  geom_boxplot(width=0.1,  position = position_dodge(width = 0.9))+
  theme_classic(base_size = 14)+
  scale_x_discrete(labels=c("A. c. canadensis", "A. c. tabida"))+
...
p
  ggsave(p, filename = paste(getwd(), "/figures/Segment_density_subspecies_sex_violin.pdf", sep=""),  bg = "transparent", height = 4, width = 4, units = "in")
...
p<-dat.s %>%
  ...
  labs(x="", y= "segment count")
p
  ggsave(p, filename = paste(getwd(), "/figures/Segment_count_subspecies_age_violin.pdf", sep=""),  bg = "transparent", height = 4, width = 4, units = "in")
...
p<-dat.s %>%
  ...
  facet_wrap(.~bursa)
p
  ggsave(p, filename = paste(getwd(), "/figures/Segment_density_subspecies_age_violin.pdf", sep=""),  bg = "transparent", height = 4, width = 4, units = "in")
...
t.test(segment_density ~ sex, data=subset(dat.l, sex %in% c("M", "F")))

t.test(segment_density~ sex, data=subset(dat.g, sex %in% c("M", "F")))
...
```{R}
#ggpairs(dat.f[,c(3:6,10:22)])
#data look pretty good! Need to trim some outliers but otherwise pretty great!

library(ggpmisc)

f <-sort(sample(dat.s[which(dat.s$sex=="F"),],size=75))

m <-sort(sample(dat.s[which(dat.s$sex=="M")],size=75))%>% pivot_wider(names_to=c("sex", "taxon_name"), values_to = "log_mtl")


M.a.c.c <- dat.f%>%filter(sex=="M", taxon_name=="Antigone canadensis canadensis")%>%sample_n(.,34)%>%arrange(mass)%>%select(taxon_name, sex, mean_trachea_length, mass)%>%rename(mtl_male=mean_trachea_length, mass_male=mass)


Fe.a.c.c <- dat.f%>%filter(sex=="F", taxon_name=="Antigone canadensis canadensis")%>%sample_n(.,34)%>%arrange(mass)%>%select(taxon_name, sex, mean_trachea_length, mass)%>%rename(mtl_female=mean_trachea_length, mass_female=mass)%>%select(mtl_female,mass_female)

sex.plot2 <- cbind(M.a.c.c, Fe.a.c.c) 


M.a.c.t <- dat.f%>%filter(sex=="M", taxon_name=="Antigone canadensis tabida")%>%sample_n(.,49)%>%arrange(mass)%>%select(taxon_name, sex, mean_trachea_length, mass)%>%rename(mtl_male=mean_trachea_length, mass_male=mass)


Fe.a.c.t <- dat.f%>%filter(sex=="F", taxon_name=="Antigone canadensis tabida")%>%sample_n(.,49)%>%arrange(mass)%>%select(taxon_name, sex, mean_trachea_length, mass)%>%rename(mtl_female=mean_trachea_length, mass_female=mass)%>%select(mtl_female,mass_female)

sex.act2 <- cbind(M.a.c.t, Fe.a.c.t) 

sex.plot2<-rbind(sex.plot2, sex.act2)

sim.sex <- sim_df(sex.plot2, n=1000, between = c("taxon_name", "sex"))

p <- ggplot(sim.sex, aes(x=log10(mtl_male), y=log10(mtl_female), color=taxon_name, fill=taxon_name))+
  geom_point(alpha=0.3, shape=21, color="transparent")+
  geom_smooth(method = "lm")+
  scale_fill_manual(values=c("steelblue", "grey53"))+
  scale_color_manual(values=c("steelblue2", "grey33"))+
  labs(x="log10(male trachea length (mm))", y="log10(female trachea length (mm))")+
  theme_classic(base_size = 14)+
...
p2 <- ggplot(sim.sex, aes(x=log10(mass_male), y=log10(mass_female), color=taxon_name, fill=taxon_name))+
  geom_point(alpha=0.5, shape=21, color="transparent")+
  geom_smooth(method = "lm")+
  scale_fill_manual(values=c("steelblue", "grey53"))+
  scale_color_manual(values=c("steelblue2", "grey33"))+
  labs(x="log10(male mass (g))", y="log10(female mass (g))")+
  theme_classic(base_size = 14)+
  theme(legend.position = "none")
...
p<-dat.s %>%
  ...
  filter(mass >= 2400,
         bursa %in% c("Y", "N"))%>%
  ggplot(., aes(resid, segment_density, fill=sex, color=sex))+
  geom_point(shape=21)+
  geom_smooth(method = "lm")+
  theme_classic(base_size = 14)+
  scale_fill_discrete_qualitative(palette = "Harmonic")+
  ...
  theme(axis.text.x = element_text(face = "italic"),
        legend.position = c(0.9,0.9))+
  labs(x="residuals", y= "segment density")+
  facet_wrap(.~taxon_name)
p
  ggsave(p, filename = paste(getwd(), "/figures/Segment_density_subspecies_scatter.pdf", sep=""),  bg = "transparent", height = 4, width = 8, units = "in")

```

Mtl by mass by taxon
```{r}
p <- ggplot(dat.s.conservative, aes(x= log_mass13, y=log_mtl, group=sex, color=sex, fill=sex)) #substitute dat.s to remove tabida outliers, leave dat.f to see them
p <- p + geom_point(shape=21, color= "black",size =2, alpha = 0.7)
p <- p + geom_smooth( fill="grey68", method="lm", alpha=0.2)
p <- p + theme_bw()
p <- p + scale_fill_manual(values = c("#56B4E9", "#E69F00", "orchid2"))
p <- p + scale_color_manual(values = c("#56B4E9", "#E69F00", "orchid2"))
...
p <- p + xlab(bquote((mass^1/2), "(g)"))
p <- p + labs(x=expression(paste("log10(",mass^frac(1,3),")", " (g)"), sep=""), y= "trachea length (mm)")
# p <- p + stat_poly_eq(aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~")),
#                label.x.npc = "right", label.y.npc = 0.15,
#                formula = formula, parse = TRUE, size = 3)
print(p)

ggsave(p, filename = "Mtl_by_mass.png", height = 6, width = 8, units = "in", bg="transparent")
...
plot1 <- ggplot(dat.g%>%filter(bursa %in% c("N", "NA"), segment_count2 <=350), aes(x = segment_count2, y =mean_trachea_length, color = sex, fill=sex)) + 
  geom_point(aes(color = sex), size = 3) + 
  geom_point(shape = 1, color = "black", size = 3) + 
  stat_smooth(method = "lm", fullrange = F) +
  scale_fill_manual(values=c("darkseagreen2", "burlywood2"))+
  scale_color_manual(values=c("darkseagreen2", "burlywood2"))+
  scale_y_continuous(name = "trachea length (mm)", limits=c(min(dat.g$mean_trachea_length -10), max(dat.g$mean_trachea_length +5)), expand = c(0,0))+
  scale_x_continuous(name = "segment count", limits= c(240, 340), expand = c(0,0)) + 
  theme_pubr() +
  theme(legend.position = c(0.15, 0.9))

dens1 <- ggplot(dat.g%>%filter(bursa %in% c("N", "NA"), segment_count2 <=350), aes(x = segment_count2, fill = sex)) + 
  geom_density(alpha = 0.4, adjust=3) + 
  scale_x_continuous(limits = c(240,340)) + 
  scale_fill_manual(values=c("darkseagreen2", "burlywood2"))+
  theme_void() + 
  theme(legend.position = "none",
...
dens2 <- ggplot(dat.g%>%filter(bursa %in% c("N", "NA"), segment_count2 <=350), aes(x = mean_trachea_length, fill = sex)) + 
  geom_density(alpha = 0.4, adjust=3,) + 
  scale_x_continuous(limits=c(min(dat.g$mean_trachea_length -20), max(dat.g$mean_trachea_length +20))) + 
  scale_fill_manual(values=c("darkseagreen2", "burlywood2"))+
  theme_void() + 
  theme(legend.position = "none") + 
  coord_flip()
...
p
ggsave(p, filename = "figures/mtl_by_seg_count_greater.pdf", height = 6, width = 6,  units = "in", bg="transparent")



plot1 <- ggplot(dat.l%>%filter(bursa %in% c("N", "NA"), segment_count2 <=350), aes(x = segment_count2, y = mean_trachea_length, color = sex, fill=sex)) + 
  geom_point(aes(color = sex), size = 3) + 
  geom_point(shape = 1, color = "black", size = 3) + 
  stat_smooth(method = "lm", fullrange = F) +
  scale_fill_manual(values=c("darkseagreen2", "burlywood2"))+
  scale_color_manual(values=c("darkseagreen2", "burlywood2"))+
  scale_y_continuous(name = "trachea length (mm)", limits=c(min(dat.l$mean_trachea_length -10), max(dat.l$mean_trachea_length +5)), expand = c(0,0))+
  scale_x_continuous(name = "segment count", limits= c(240, 340), expand = c(0,0)) + 
  theme_pubr() +
  theme(legend.position = c(0.15, 0.9))

dens1 <- ggplot(dat.l%>%filter(bursa %in% c("N", "NA"), segment_count2 <=350), aes(x = segment_count2, fill = sex)) + 
  geom_density(alpha = 0.4, adjust=3) + 
  scale_x_continuous(limits = c(240,340)) + 
  scale_fill_manual(values=c("darkseagreen2", "burlywood2"))+
  theme_void() + 
...
dens2 <- ggplot(dat.l%>%filter(bursa %in% c("N", "NA"), segment_count2 <=350), aes(x = mean_trachea_length, fill = sex)) + 
  geom_density(alpha = 0.4, adjust=3) + 
  scale_x_continuous(limits=c(min(dat.l$mean_trachea_length -10), max(dat.l$mean_trachea_length +5))) + 
  scale_fill_manual(values=c("darkseagreen2", "burlywood2"))+
  theme_void() + 
  theme(legend.position = "none") + 
  coord_flip()
...
p<-dens1 + plot_spacer() + plot1 + dens2 + 
  plot_layout(ncol = 2, nrow = 2, widths = c(4, 1), heights = c(1, 4))
p
ggsave(p, filename = "figures/mtl_by_seg_count_lesser.pdf", height = 6, width = 6,  units = "in", bg="transparent")
```

Interesting that the slope relationship breaks down only for male lesser sandhill cranes...

Let's try and look at this first by using `faux` to simulate crane sex data and run 1000 linear models to determine the slope of each

```{r, include =F}
#define iterations
iter <- 2000
sim.lm <-data.frame(matrix(NA, nrow=iter, ncol=5))
colnames(sim.lm) <- c("taxon_name", "sex", "r2", "coeff", "p")

for(i in 1:iter){
  sim.dat.l.m <- sim_df(data=dat.l.m%>%dplyr::select(log_mass, log_mtl), n=34, id="taxon_name")
  
  sim.l.m.lm <- lm(log_mtl~log_mass, data=sim.dat.l.m)
  
  sim.lm$taxon_name[i] <- dat.l.m$taxon_name[1]
    sim.lm$sex[i] <- dat.l.m$sex[1]
    sim.lm$r2[i] <- summary(sim.l.m.lm)$r.squared
    sim.lm$coeff[i] <-sim.l.m.lm$coefficients[2]
  sim.lm$p[i] <- summary(sim.l.m.lm)$coefficients[,4][2]  
  
    sim.dat.l.f <- sim_df(data=dat.l.f%>%dplyr::select(log_mass, log_mtl), n=34, id="taxon_name")
  
  sim.l.f.lm <- lm(log_mtl~log_mass, data=sim.dat.l.f)
  
  sim.lm$taxon_name[i] <- "Antigone canadensis"
    sim.lm$sex[i] <- "F"
    sim.lm$r2[i] <- summary(sim.l.f.lm)$r.squared
    sim.lm$coeff[i] <-sim.l.f.lm$coefficients[2]
  sim.lm$p[i] <- summary(sim.l.f.lm)$coefficients[,4][2]  
...
```{r}
ggplot(dat.s, aes(x = log(prop_trachea) , y = log(mass), colour = sex))+
    geom_point(size =2, alpha = 1)+
    geom_smooth(aes(group=sex),method="lm")+
    scale_colour_manual(values = c("#56B4E9", "#E69F00","orchid2"))+
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
    theme(axis.text=element_text(size=16),
        axis.title=element_text(size=18))+
    facet_wrap(~taxon_name)

#Notice strong negative relationship in tabida is maybe absent in canadensis unless three furthers right candensis are due to measurement error
#They show up again in here...

p <- ggplot(dat.s, aes(x = log(mean_trachea_length) , y = log(segment_density), colour = sex))
p <- p + geom_point(size =2, alpha = 0.7)
p <- p + geom_smooth(aes(group=sex), method="lm")
p <- p + scale_colour_manual(values = c("#56B4E9", "#E69F00","orchid2"))
p <- p + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
p <- p + theme(axis.text=element_text(size=16),
        axis.title=element_text(size=18))
p <- p + facet_wrap(~taxon_name)
print(p)

#more segments in smaller trachea but proportionally longer when accounting for body size
#clear segment density outliers maybe need to recount segments for those ~2 points.

p <- ggplot(dat.s, aes(x = log(mean_trachea_length) , y = log(segment_count2), colour = sex))
p <- p + geom_point(size =2, alpha = 0.7)
p <- p + geom_smooth(aes(group=sex), method="lm")
p <- p + scale_colour_manual(values = c("#56B4E9", "#E69F00","orchid2"))
p <- p + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
p <- p + theme(axis.text=element_text(size=16),
        axis.title=element_text(size=18))
p <- p + facet_wrap(~taxon_name)
p <- p + labs(x="mean trachea length (mm)", y= "number of segments")
print(p)

library(ggplot2)
p <- ggplot(dat.s, aes(x = log(poserior_width), y = log(mean_trachea_length), colour = sex))
p <- p + geom_point(size =2, alpha = 0.7)
p <- p + geom_smooth(aes(group=sex), method="lm")
p <- p + scale_colour_manual(values = c("#56B4E9", "#E69F00","orchid2"))
p <- p + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
#no surprise tracheal width positively varies with tracheal length but much overlap between species

library(ggplot2)
p <- ggplot(dat.f, aes(x = wing_chord , y = mean_trachea_length, colour = taxon_name, shape = sex))
p <- p + geom_point(size =3)

p <- p + scale_colour_manual(values = c("grey55", "orangered3"))
p <- p + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
p <- p + theme(axis.text=element_text(size=16),
        axis.title=element_text(size=18))
p <- p + xlim(390,600) #ther is really small outlier in wing chord
print(p)

library(ggplot2)
p <- ggplot(dat.s, aes(x = tarsus, y = mean_trachea_length, colour = taxon_name, shape = sex))
p <- p + geom_point(size =3)
...
#Look at young birds only

p <- ggplot(dat.baby, aes(x = mean_trachea_length , y = mass, colour = sex)) #substitute dat.s to remove tabida outliers, leave dat.f to see them
p <- p + geom_point(size =4, alpha = 0.5)
p <- p + facet_grid(. ~ taxon_name)
p <- p + scale_colour_manual(values = c("#56B4E9", "#E69F00", "orchid2"))
p <- p + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
p <- p + theme(axis.text=element_text(size=16),
        axis.title=element_text(size=18))
...
#Notice three tabida that may be misclassified

p <- ggplot(dat.baby, aes(x = prop_trachea , y = mass, colour = bursa, shape = taxon_name))
...
p <- ggplot(dat.baby, aes(x = mean_trachea_length , y = segment_density, colour = taxon_name, shape = sex))
p <- p + geom_point(size =3)
...
#more segments in smaller trachea btu proportionally longer when accounting for body size
#clear segemnt density outliers maybe need to recount segments for those ~2 points.

p <- ggplot(dat.baby, aes(x = mean_trachea_length , y = segment_count2, colour = bursa, shape = sex))
p <- p + geom_point(size =3)
p <- p + facet_grid(. ~ taxon_name)
#p <- p + scale_colour_manual(values = c("grey55", "orangered3"))
p <- p + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
library(ggplot2)
p <- ggplot(dat.baby, aes(x = poserior_width , y = mean_trachea_length, colour = taxon_name, shape = sex))
p <- p + geom_point(size =3)
...
library(ggplot2)
p <- ggplot(dat.baby, aes(x = wing_chord , y = mean_trachea_length, colour = taxon_name, shape = sex))
p <- p + geom_point(size =3)
...
library(ggplot2)
p <- ggplot(dat.baby, aes(x = tarsus, y = mean_trachea_length, colour = taxon_name, shape = sex))
p <- p + geom_point(size =3)
...
#3d scatter plot
scatter3d(x = dat.s$log_mass, y = dat.s$log_mtl, z = log(dat.s$segment_count2), groups=dat.s$taxon_name, grid=FALSE, fit = "smooth", surface=FALSE)
```
...
#slopes by species
boot.s.l <- boot.slope.bca(dat.l$log_mtl, dat.l$log_mass, null.hyp = NULL, R = 10000)
boot.s.g <- boot.slope.bca(dat.g$log_mtl, dat.g$log_mass, null.hyp = NULL, R = 10000)
...
lm.g.b <- boot(data=dat.g, statistic=bs,
   R=10000, formula=log_mtl~log_mass)
lm.l.b <- boot(data=dat.l, statistic=bs,
   R=10000, formula=log_mtl~log_mass)


boot.df <-data.frame(matrix(NA, nrow=20000, ncol=3))

boot.df$X3 <-c(lm.l.b$t[,1], lm.g.b$t[,1])
boot.df$X2<-c(boot.s.l$Boot.values, boot.s.g$Boot.values)
boot.df$X1 <- rep(c("lesser", "greater"), each =10000)
```

## Plot Bootstrapped Slope

```{R}
p.s <-ggplot(boot.df, aes(x = X2, y = X1, fill = X1)) +
        #geom_density_ridges(alpha = 0.7)+
        stat_density_ridges(quantile_lines = TRUE, quantiles = 2, alpha = 0.7)+
        labs(x="boostrapped slope(log trachea length ~ log mass)",y="subspecies")+
        scale_fill_manual(values = c("dodgerblue2", "slateblue2"))+
        theme(axis.text=element_text(size=16),
        axis.title=element_text(size=18),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "none")
plot(p.s)


grid.arrange(p.i, p.s, nrow=2)
```

```{R}
#slopes by sex and species
boot.s.l.m <- boot.slope.bca(dat.l.m$log_mtl, dat.l.m$log_mass, null.hyp = NULL, R = 10000)
boot.s.l.f <- boot.slope.bca(dat.l.f$log_mtl, dat.l.f$log_mass, null.hyp = NULL, R = 10000)
boot.s.g.m <- boot.slope.bca(dat.g.m$log_mtl, dat.g.m$log_mass, null.hyp = NULL, R = 10000)
boot.s.g.f <- boot.slope.bca(dat.g.f$log_mtl, dat.g.f$log_mass, null.hyp = NULL, R = 10000)

#Intercepts
# Bootstrap 95% CI for regression coefficients
# function to obtain regression weights
#Already wrote function above

# bootstrapping with 10000 replications
lm.g.m.b <- boot(data=dat.g.m, statistic=bs,
   R=10000, formula=log_mtl~log_mass)
lm.g.f.b <- boot(data=dat.g.f, statistic=bs,
   R=10000, formula=log_mtl~log_mass)
lm.l.m.b <- boot(data=dat.l.m, statistic=bs,
   R=10000, formula=log_mtl~log_mass)
lm.l.f.b <- boot(data=dat.l.f, statistic=bs,
   R=10000, formula=log_mtl~log_mass)


boot.l.df <-data.frame(matrix(NA, nrow=20000, ncol=3))

boot.l.df$X3 <-c(lm.l.m.b$t[,1], lm.l.f.b$t[,1])
boot.l.df$X2<-c(boot.s.l.m$Boot.values, boot.s.l.f$Boot.values)
boot.l.df$X1 <- rep(c("male", "female"), each =10000)

boot.g.df <-data.frame(matrix(NA, nrow=20000, ncol=3))

boot.g.df$X3 <-c(lm.g.m.b$t[,1], lm.g.f.b$t[,1])
boot.g.df$X2<-c(boot.s.g.m$Boot.values, boot.s.g.f$Boot.values)
boot.g.df$X1 <- rep(c("male", "female"), each =10000)

p.s.l <-ggplot(boot.l.df, aes(x = X2, y = X1, fill = X1)) +
        #geom_density_ridges(alpha = 0.7)+
        stat_density_ridges(quantile_lines = TRUE, quantiles = 2, alpha = 0.7)+
        labs(x="slope (log trachea length ~ log mass)",y="sex")+
        scale_fill_manual(values = c("#56B4E9", "#E69F00", "orchid2"))+
...
p.s.g <-ggplot(boot.g.df, aes(x = X2, y = X1, fill = X1)) +
        #geom_density_ridges(alpha = 0.7)+
        stat_density_ridges(quantile_lines = TRUE, quantiles = 2, alpha = 0.7)+
        labs(x="slope (log trachea length ~ log mass)",y="sex")+
        scale_fill_manual(values = c("#56B4E9", "#E69F00", "orchid2"))+
...
p.i.l <-ggplot(boot.l.df, aes(x = X3, y = X1, fill = X1)) +
        #geom_density_ridges(alpha = 0.7)+
        stat_density_ridges(quantile_lines = TRUE, quantiles = 2, alpha = 0.7)+
        labs(x=" intercept (log trachea length ~ log mass)",y="sex")+
        ggtitle("Lesser")+
        coord_cartesian(clip = "off") +
        scale_fill_manual(values = c("#56B4E9", "#E69F00", "orchid2"))+
        theme(axis.text=element_text(size=10), plot.title = element_text(hjust = 0.5),
        axis.title=element_text(size=12),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "none")+
     theme_classic()
plot(p.i.l)

p.i.g <-ggplot(boot.g.df, aes(x = X3, y = X1, fill = X1)) +
        #geom_density_ridges(alpha = 0.7)+
        stat_density_ridges(quantile_lines = TRUE, quantiles = 2, alpha = 0.7)+
        labs(x="intercept (log trachea length ~ log mass)",y="sex")+
        ggtitle("Greater")+
...
ggsave(p.s.g, filename = "greater_slope_mass_MTL.png", height = 6, width = 8, units = "in", bg="transparent")

ggsave(p.s.l, filename = "lesser_slope_mass_MTL.png", height = 6, width = 8, units = "in", bg="transparent")


grid.arrange(p.i.l,p.i.g, p.s.l, p.s.g, nrow=2)

g <-grid.arrange(p.i.l,p.i.g, nrow=2)
ggsave(g, filename = "intercept_mass_MTL.png", height = 6, width = 8, units = "in", bg="transparent")

#Lets do anova of slopes
#lesser
lm.s.l <- lm(log_mtl ~ log_mass*sex, data=dat.l)
summary(lm.s.l)
anova(lm.s.l)
simple_slopes(lm.s.l)

# Obtain slopes
lm.s.l$coefficients
lm.s.l.lst <- lstrends(lm.s.l, "sex", var="log_mass")
lm.s.l.lst
...
lm.s.g <- lm(log_mtl ~ log_mass*sex, data=dat.g)
anova(lm.s.g)

# Obtain slopes
lm.s.g$coefficients
lm.s.g.lst <- lstrends(lm.s.g, "sex", var="log_mtl")
lm.s.g.lst
simple_slopes(lm.s.g)

# Compare slopes
pairs(lm.s.g.lst)

lm.f <- lm(log_mtl ~log_mass*sex*taxon_name, data=dat.f)
anova(lm.f)

# Obtain slopes
lm.f$coefficients
lm.s.f.lst <- lstrends(lm.f, "sex", var="log_mass")
lm.s.f.lst
...
ggsave(g, filename = "Crane_morph_PCA.pdf", height = 6, width = 8, units = "in")
...
g2 <- ggbiplot(MP.pca2, obs.scale = 1, var.scale = 1, ellipse = TRUE, 
              groups = MP.sp2[,2], circle = FALSE)
g2 <- g2 + geom_point(aes(color = factor(MP.sp2[,2])), size =2, alpha=0.2)
...
ggsave(g, filename = "Crane_Trachea_morph_PCA.pdf", height = 6, width = 8, units = "in")
...
qqPlot(dat.s$segment_density) #outlier(s)? probably those high counts in tabida
hist(dat.s$segment_density)
...
l.m.i <-coef(mod.l.m)[1] + min.l.f*coef(mod.l.m)[2]
l.f.i <-coef(mod.l.f)[1] + min.l.f*coef(mod.l.f)[2]
...
save(m0.g, file=paste(getwd(),"/models/", "crane_trachea_greater_intercept_only_fam_gaussian.RData", sep="")) # save model

#load("crane_trachea_greater_intercept_only_fam_gaussian.RData") # If loading from pre-saved file and not re-running
...
# export predictions
predict(m0.g, summary=FALSE, ndraws = 100) %>% 
  write.csv("greaters_int_only_dist_draws.csv")
...
save(m1.a.g,file=paste(getwd(), "/models/", "crane_trachea_greater_additive_no_sex_fam_gaussian.RData", sep="")) # save model

#load("crane_trachea_greater_additive_no_sex_fam_gaussian.RData") # If loading from pre-saved file and not re-running
...
save(m2.a.g,file=paste(getwd(), "/models/", "crane_trachea_greater_additive_full_fam_gaussian.RData", sep="")) # save model

#load("crane_trachea_greater_additive_full_fam_gaussian.RData") # If loading from pre-saved file and not re-running
...
m3.i.g <- brm(
  formula = bf(log_mtl ~ 1 + (log_mass.z + sex)^2),
  data = dat.g,
  family = skew_normal(),
  cores = 4,
  chains = 4,
...
save(m3.i.g, file=paste(getwd(), "/models/", "crane_trachea_full_interaction_greater_fam_gaussian.RData", sep="")) # save model

#load(paste(getwd(), "/models/", "crane_trachea_full_interaction_greater_fam_gaussian.RData", sep="")) # If loading from pre-saved file and not re-running
...
save(m0.l, file=paste(getwd(), "/models/", "crane_trachea_lesser_intercept_only_fam_gaussian.RData", sep="")) # save model

#load("crane_trachea_lesser_intercept_only_fam_student.RData") # If loading from pre-saved file and not re-running
...
save(m1.a.l, file=paste(getwd(), "/models/", "crane_trachea_lesser_additive_no_sex_fam_gaussian.RData", sep="")) # save model

#load("crane_trachea_lesser_additive_no_sex_fam_student.RData re-running
...
save(m2.a.l, file=paste(getwd(), "/models/crane_trachea_lesser_additive_mass_sex_fam_gaussian.RData", sep="")) # save model

#load("crane_trachea_lesser_additive_full_fam_gaussian.RData") # If loading from pre-saved file and not re-running
...
save(m3.i.l, file=paste(getwd(), "/models", "crane_trachea_full_interaction_lessers_fam_gaussian.RData", sep="")) # save model

#load(paste(getwd(), "/models", "crane_trachea_full_interaction_lessers_fam_gaussian.RData", sep="")) # If loading from pre-saved file and not re-running
...
# Model comparisons:  Best model for greaters is additive then interaction... maybe model average?
# Model comparisons:
#        elpd_diff se_diff
# m3.i.g   0.0       0.0  
# m2.a.g  -0.1       1.4  
...
# m1.a.l  -4.9       4.0  
# m0.l   -23.4       5.7  
#Interaction is very close to additive so maybe jsutified to use that one.


# Calculate Bayesian R^2:
bayes_R2(m0.g) # 0.0000000000 nothing
bayes_R2(m1.a.g) # 0.479
bayes_R2(m2.a.g) #0.500
bayes_R2(m3.i.g) # 0.51
bayes_R2(m0.l)  #0.0000000000 nothing
bayes_R2(m1.a.l) # 0.44
bayes_R2(m2.a.l) #0.54
bayes_R2(m3.i.l) # 0.54
...
tidy_pred.f <- m3.i.g %>% 
  predicted_draws(newdata = newdata.f )
...
tidy_pred.m <- m3.i.g %>% 
  predicted_draws(newdata = newdata.m )

tidy.pred.contrasts <- data.frame(contrast = (tidy_pred.m$.prediction - tidy_pred.f$.prediction),
                                  SDI =(tidy_pred.m$.prediction - tidy_pred.f$.prediction)/tidy_pred.f$.prediction,
                                  log_mass.z = tidy_pred.f$log_mass.z)

p<-tidy.pred.contrasts%>%
  ggplot(., aes(log_mass.z, contrast))+
  stat_ribbon(alpha=0.6, .width = c(0.5, 0.6, 0.7, 0.8, 0.9))+
  scale_fill_brewer(palette = "Greys")+
  scale_color_brewer(palette = "Greys")+
  geom_hline(yintercept = 0, linetype="dashed")+
  annotate("rect", xmin = -1.079172, xmax = 0.7982938, ymin = -0.10, ymax =  0.23,
           alpha = .1,fill = "blue")+
  labs(x=expression(paste("standardized(log"["10"], "mass)", sep="")), y="tracheal length contrast (M-F)")+
  scale_y_continuous(limits = c(-0.10, 0.23))+
  theme_cowplot()
p
ggsave(p, filename=paste(figures, "greater_contrasts_by_weight.pdf", sep=""), width=5.3, height=4)
...
p<-tidy.pred.contrasts%>%
  ...
  scale_color_brewer(palette = "Greys")+
  geom_hline(yintercept = 0, linetype="dashed")+
  annotate("rect", xmin = -1.079172, xmax = 0.7982938, ymin = -0.035, ymax =  0.085,
           alpha = .1,fill = "blue")+
  labs(x=expression(paste("standardized(log"["10"], "mass)", sep="")), y=expression(paste("SDI"["tracheal length"])))+
  scale_y_continuous(limits = c(-0.035, 0.085))+
  theme_cowplot()
p
ggsave(p, filename=paste(figures, "greater_SDI_by_weight.pdf", sep=""), width=5.3, height=4)
```
### Lessers
```{r}
...
p<-tidy.pred.contrasts%>%
  ...
  scale_color_brewer(palette = "Greys")+
  geom_hline(yintercept = 0, linetype="dashed")+
  annotate("rect", xmin = -1.169667, xmax = 0.9255056, ymin = -0.10, ymax =  0.23,
           alpha = .1,fill = "blue")+
  labs(x=expression(paste("standardized(log"["10"], "mass)", sep="")), y="tracheal length contrast (M-F)")+
  scale_y_continuous(limits = c(-0.10, 0.23))+
  theme_cowplot()
...
p
ggsave(p, filename=paste(figures, "lesser_contrasts_by_weight.pdf", sep=""), width=5.3, height=4)
...
p<-tidy.pred.contrasts%>%
  ...
  scale_color_brewer(palette = "Greys")+
  geom_hline(yintercept = 0, linetype="dashed")+
  annotate("rect", xmin = -1.169667, xmax = 0.9255056, ymin = -0.035, ymax =  0.085,
           alpha = .1,fill = "blue")+
  labs(x=expression(paste("standardized(log"["10"], "mass)", sep="")), y=expression(paste("SDI"["tracheal length"])))+
  scale_y_continuous(limits = c(-0.035, 0.085))+
  theme_cowplot()
p
ggsave(p, filename=paste(figures, "lesser_SDI_by_weight.pdf", sep=""), width=5.3, height=4)
...
SDI.plot <- data.frame(x=SDI.pred.g, Taxon="greater")
SDI.plot <- rbind(SDI.plot, data.frame(x=SDI.pred.l, Taxon="lesser"))
SDI.plot%>%
  group_by(Taxon)%>%
...
p
ggsave(p, filename = paste(getwd(), "/figures/", "SDI_PP_trachea_grey.pdf", sep=""), width=6, height=5)
...
save(m.c.n.l, file=paste(getwd(), "/models/", "crane_mass_lesser_catagorical_null_fam_skew_normal.RData", sep="")) # save model

#load("crane_mass_lesser_catagorical_null_fam_gaussian.RData") # If loading from pre-saved file and not re-running
...
save(m.c.s.l, file=paste(getwd(), "/models/", "crane_mass_lesser_catagorical_sex_only_fam_skew_normal.RData", sep="")) # save model

#load("crane_mass_lesser_catagorical_sex_only_fam_gaussian.RData") # If loading from pre-saved file and not re-running
...
save(m.c.n.g, file=paste(getwd(), "/models/", "crane_mass_greater_catagorical_null_fam_gaussian.RData", sep="")) # save model

#load("crane_mass_greater_catagorical_null_fam_gaussian.RData") # If loading from pre-saved file and not re-running
...
save(m.c.s.g, file=paste(getwd(), "/models/", "crane_mass_greater_catagorical_sex_only_fam_gaussian.RData", sep="")) # save model

#load("crane_mass_greater_catagorical_sex_only_fam_gaussian.RData") # If loading from pre-saved file and not re-running
...
p<-ggplot(SDI.plot, aes(x=x,y=Taxon, colour = cat,
                  fill = cat))+
  ...
  scale_x_continuous(limits=c(-0.002, 0.03))+
    labs(x=expression(paste("SDI"[italic("mass")])), y="")
p

ggsave(p, filename = paste(getwd(), "/figures/", "SDI_cat_by_color.pdf", sep=""), width=6, height=5)
...
ggsave(p, filename = paste(getwd(), "/figures/", "SDI_contrasts_grey.pdf", sep=""), width=6, height=5)
...
p <-ggplot(samp.l.f, aes(x=log_mass, log_mtl, linetype=taxon_name, group=factor(i)))+
    stat_smooth( geom= "line",size=0.5, color ="grey40", alpha=0.3, method="lm", se=T)+
      ...
      theme(legend.position = c(0.3,0.85),
            legend.title = element_blank())
ggsave(p, filename= paste(figures,"lm_replicates_with_faux_simulated_datasets.pdf", sep=""), device="pdf", bg = "transparent", height = 4, width = 6, units = "in")
...
l<-dat.l %>%
  ...
  theme(legend.title = element_blank(),
        legend.position = "none")
  ggsave(l, filename = paste(getwd(), "/figures/Epredictions_lessers_SDImass.pdf", sep=""), width=5, height = 5)
...
ggsave(g, filename = paste(getwd(), "/figures/Epredictions_greaters_SDImass.pdf", sep=""), width=5, height = 5)
...
save(seg.l.int.only, file=paste(getwd(), "/models/", "crane_trachea_segments_lesser_int_only_fam_poisson.RData", sep="")) # save model

#load("crane_trachea_segments_lesser_int_only_fam_gaussian.RData") # If loading from pre-saved file and not re-running
...
#use this to get idea of priors then set your own!!
# priors <-get_prior(log_mtl ~ 1 + log10(segment_count2)  + sex,
#                    data = dat.l, family = gaussian())

seg1.l.a.s <- brm(
  formula = bf(segment_count2 ~ 1 + sex),
  data = dat.l,
  family = poisson(),
  cores = 4,
  chains = 4,
...
save(seg1.l.a.s, file=paste(getwd(), "/models/", "crane_trachea_segments_lesser_additive_sex_fam_poisson.RData", sep="")) # save model

#load("crane_trachea_segments_lesser_additive_sex_fam_gaussian.RData") # If loading from pre-saved file and not re-running
...
seg1.l.a.b <- brm(
  formula = bf(segment_count2 ~ 1 + bursa),
  data = dat.l,
  family = poisson(),
  cores = 4,
  chains = 4,
...
save(seg1.l.a.b, file=paste(getwd(), "/models/", "crane_trachea_segments_lesser_additive_bursa_fam_poisson.RData", sep="")) # save model

#load("crane_trachea_segments_lesser_additive_sex_fam_poisson.RData") # If loading from pre-saved file and not re-running
...
seg1.l.a.t <- brm(
  formula = bf(segment_count2 ~ 1 + log_mtl),
  data = dat.l,
  family = poisson(),
  cores = 4,
  chains = 4,
...
save(seg1.l.a.t, file=paste(getwd(), "/models/", "crane_trachea_segments_lesser_additive_mtl_fam_poisson.RData", sep="")) # save model
...
seg2.l.a.sb <- brm(
  formula = bf(segment_count2 ~ 1 + sex + bursa),
  data = dat.l,
  family = poisson(),
  cores = 4,
  chains = 4,
...
save(seg2.l.a.sb, file=paste(getwd(), "/models/", "crane_trachea_segments_lesser_additive_bursa-sex_fam_poisson.RData", sep="")) # save model

#load("crane_trachea_segments_lesser_additive_bursa-sex_fam_poisson.RData") # If loading from pre-saved file and not re-running
...
seg2.l.a.st <- brm(
  formula = bf(segment_count2 ~ 1 + sex + log_mtl),
  data = dat.l,
  family = poisson(),
  cores = 4,
  chains = 4,
...
save(seg2.l.a.st, file=paste(getwd(), "/models/", "crane_trachea_segments_lesser_additive_sex-mtl_fam_poisson.RData", sep="")) # save model
...
seg2.l.a.bt <- brm(
  formula = bf(segment_count2 ~ 1 + bursa + log_mtl),
  data = dat.l,
  family = poisson(),
  cores = 4,
  chains = 4,
...
save(seg2.l.a.bt, file=paste(getwd(), "/models/", "crane_trachea_segments_lesser_additive_bursa-mtl_fam_poisson.RData", sep="")) # save model
...
seg3.l.a.tbs <- brm(
  formula = bf(segment_count2 ~ 1 + log_mtl + bursa + sex),
  data = dat.l,
  family = poisson(),
  cores = 4,
  chains = 4,
...
save(seg3.l.a.tbs, file=paste(getwd(), "/models/", "crane_trachea_segments_lesser_additive_full_fam_poisson.RData", sep="")) # save model
...
seg2.l.i.s.t <- brm(
  formula = bf(segment_count2 ~ 1 + (sex + log_mtl)^2),
  data = dat.l,
  family = poisson(),
  cores = 4,
  chains = 4,
...
save(seg2.l.i.s.t, file=paste(getwd(), "/models/", "crane_trachea_segments_lesser_interaction_sex-seg_dens_fam_poisson.RData", sep="")) # save model
...
g<-dat.l %>%
  ...
  theme(legend.title = element_blank(),
        legend.position = "none")
g
ggsave(g, filename = paste(getwd(), "/figures/Epredictions_greaters_seg_mtl.pdf", sep=""), width=5, height = 5)
...
seg.g.int.only <- brm(
  formula = bf(segment_count2 ~ 1),
  data = dat.g%>%filter(segment_count2<350),
  family = gaussian(),
  cores = 4,
  chains = 4,
  thin = 10, #chop out transitions?
...
save(seg.g.int.only, file=paste(getwd(), "/models/", "crane_trachea_segments_greater_int_only_fam_gaussian.RData", sep="")) # save model
...
#use this to get idea of priors then set your own!!
# priors <-get_prior(log_mtl ~ 1 + log10(segment_count2)  + sex,
#                    data = dat.g%>%filter(segment_count2 <350), family = gaussian())
...
save(seg1.g.a.s, file=paste(getwd(), "/models/", "crane_trachea_segments_greater_additive_sex_fam_gaussian.RData", sep="")) # save model

#load("crane_trachea_segments_greater_additive_sex_fam_gaussian.RData") # If loading from pre-saved file and not re-running
...
save(seg1.g.a.b, file=paste(getwd(), "/models/", "crane_trachea_segments_greater_additive_bursa_fam_gaussian.RData", sep="")) # save model

#load("crane_trachea_segments_greater_additive_sex_fam_poisson.RData") # If loading from pre-saved file and not re-running
...
save(seg1.g.a.t, file=paste(getwd(), "/models/", "crane_trachea_segments_greater_additive_mtl_fam_gaussian.RData", sep="")) # save model
...
save(seg2.g.a.sb, file=paste(getwd(), "/models/", "crane_trachea_segments_greater_additive_bursa-sex_fam_gaussian.RData", sep="")) # save model

#load("crane_trachea_segments_greater_additive_bursa-sex_fam_poisson.RData") # If loading from pre-saved file and not re-running
...
save(seg2.g.a.st, file=paste(getwd(), "/models/", "crane_trachea_segments_greater_additive_sex-mtl_fam_gaussian.RData", sep="")) # save model
...
save(seg2.g.a.bt, file=paste(getwd(), "/models/", "crane_trachea_segments_greater_additive_bursa-mtl_fam_gaussian.RData", sep="")) # save model
...
save(seg3.g.a.tbs, file=paste(getwd(), "/models/", "crane_trachea_segments_greater_additive_full_fam_gaussian.RData", sep="")) # save model
...
seg2.g.i.sb <- brm(
  formula = bf(segment_count2 ~ 1 + (sex + bursa)^2),
  data = dat.g%>%filter(segment_count2 <350),
  family = gaussian(),
  cores = 4,
  chains = 4,
...
save(seg2.g.i.sb, file=paste(getwd(), "/models/", "crane_trachea_segments_greater_interaction_sex-bursa_fam_gaussian.RData", sep="")) # save model
...
save(seg2.g.i.s.t, file=paste(getwd(), "/models/", "crane_trachea_segments_greater_interaction_sex-mtl_fam_gaussian.RData", sep="")) # save model
