...
```
simple additive model is top supported model.
...
```
Full additive model (F3) distributional is best for lessers.
...
p <- ggplot(ce, aes(sex, estimate__, fill=sex))+
  geom_pointrange(aes(ymin=lower__, ymax=upper__), shape=21, size=2)+
  scale_fill_sex("sexes")+
  labs( x=expression(log["10"](mass~(g))), y=expression(log["10"]("mean trachea length (mm)")))
p
...
ggsave(p, filename = paste0(figures, "SDI_cat_faceted.conservative.pdf"), width=7, height=3.5, device=cairo_pdf)
...
ggsave(p, filename = paste(getwd(), "/figures/", "SDI_contrasts_grey.conservative.pdf", sep=""), width=6, height=5, device=cairo_pdf)
