...
ggsave(g +theme(), filename = paste0(figures,"Crane_trach_PCA_sex_conservative.pdf"), height = 5.5, width = 5.5, units = "in", device = cairo_pdf)
...
p2 <- ggplot(cel$`segment_count.z:sex`, aes(effect1__, estimate__))+
  geom_ribbon(aes(ymin=lower__, ymax=upper__), fill="grey")+
  geom_line(color="black", linetype="dashed")+
 labs( x="scaled ring count ", y="residuals(trachea length ~ mass)")

p1 <- ggplot(ceg$`segment_count.z:sex`, aes(effect1__, estimate__)) +
  geom_ribbon(aes(ymin = lower__, ymax = upper__), fill = "grey") +
  geom_line(color = "black", linetype = "dashed") +
