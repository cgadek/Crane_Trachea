...
```
NOTE there is a oddly small tarsus measurement for one lesser female. If we want to do soemthing with tarsus we may want to remove...
...
p
  ggsave(p + theme(), filename = paste(getwd(), "/figures/Proportion_trachea_subspecies_sex_boxplot_conservative.pdf", sep=""),  bg = "transparent", height = 4.5, width = 5, units = "in")
...
p
ggsave(p + theme(), filename = "figures/mtl_by_seg_width_greater.conservative.pdf", height = 6, width = 6,  units = "in", device = cairo_pdf)
...
p<-dens1 + plot_spacer() + plot1 + dens2 + 
  plot_layout(ncol = 2, nrow = 2, widths = c(4, 1), heights = c(1, 4))
p
ggsave(p, filename = "figures/mtl_by_seg_count_lesser.conservative.pdf", height = 6, width = 6,  units = "in", device = cairo_pdf)
```
...
p<-dens1 + plot_spacer() + plot1 + dens2 + 
  plot_layout(ncol = 2, nrow = 2, widths = c(4, 1), heights = c(1, 4))
p
ggsave(p, filename = "figures/mtl_by_seg_width_greater.conservative.pdf", height = 6, width = 6,  units = "in", device = cairo_pdf)
...
mds.plot <- dat.s%>%
  filter(bursa=="N")%>%
  dplyr::select(3:4,6, 10:17,20)%>%
  unite('id', 1:3, remove = F)%>%
  column_to_rownames(., var="id")%>%
  na.omit()%>%
  group_by(taxon_name.x, sex)%>%
  summarise(
            across(2:10, ~ log10(.x +0.000001)))
...
mds$taxon_name <- mds.plot$taxon_name.x
# Plot MDS
...
pca.cols.to.add <-dat.s%>%
  dplyr::select(4, 6, 12:17,31:33)%>%
  na.omit()%>%
  dplyr::select(1:2)

pca.f<-dat.s%>%
  dplyr::select(12:17,31:33)%>%
  na.omit()%>%
  prcomp(., center=T,
         scale=T)
...
ggsave(g +theme(), filename = paste0(figures,"Crane_trach_PCA.conservative.pdf"), height = 4.2, width = 4.5, units = "in", device = cairo_pdf)
```
...
pca.cols.to.add <-dat.s%>%
  filter(taxon_name == "Antigone canadensis tabida")%>%
  dplyr::select(4, 6,29, 12:17,31:33)%>%
  na.omit()%>%
  dplyr::select(1:3)
...
pca.centroids <- aggregate(pca.data[,1:9], list(Type = pca.data$sex), mean)
...
l.m.r.seg <- lm(r ~ segment_count1.z + segment_width.z, data=dat.l%>%filter(sex=="M"))

l.m.r.seg

l.f.r.seg <- lm(r ~ segment_count1.z + segment_width.z, data=dat.l%>%filter(sex=="F"))

l.f.r.seg

g.m.r.seg <- lm(r ~ segment_count1.z + segment_width.z, data=dat.g%>%filter(sex=="M"))

g.m.r.seg

g.f.r.seg <- lm(r ~ segment_count1.z + segment_width.z, data=dat.g%>%filter(sex=="F"))
...
color_scheme_set(scheme = "darkgray")
pp_check(greater_mass_models[[3]][[4]], ndraws = 100) 
pp_check(greater_mass_models[[3]][[4]], type = "stat", stat = 'median', ndraws = 2000)
pp_check(greater_mass_models[[3]][[4]], type = "stat", stat = 'mean', ndraws = 2000)
...
color_scheme_set(scheme = "darkgray")
pp_check(lesser_mass_models[[3]][[4]], ndraws = 100) 
pp_check(lesser_mass_models[[3]][[4]], type = "stat", stat = 'median', ndraws = 2000)
pp_check(lesser_mass_models[[3]][[4]], type = "stat", stat = 'mean', ndraws = 2000)
...
greater_mass_models[[3]][[5]]

```
Interaction model is top supported model.

Bayes R^2
```{r}

greater_mass_models[[3]][[6]][[1]]
greater_mass_models[[3]][[6]][[2]]
greater_mass_models[[3]][[6]][[3]]
greater_mass_models[[3]][[6]][[4]]
...
lesser_mass_models[[3]][[5]]

```
Full additive model (F3) is best for lessers, though interaction model is close and is AIC rules apply for elpd then they are equivalent.


Bayes R^2
```{r}

lesser_mass_models[[3]][[6]][[1]]
lesser_mass_models[[3]][[6]][[2]]
lesser_mass_models[[3]][[6]][[3]]
lesser_mass_models[[3]][[6]][[4]]
...
ce <- conditional_effects(greater_mass_models[[3]][[4]])
...
ce <- conditional_effects(lesser_mass_models[[3]][[4]])
...
tidy_pred <- greater_mtl_models[[3]][[3]] %>% 
...
tidy_pred.f <- greater_mtl_models[[3]][[3]] %>% 
...
tidy_pred.m <- greater_mtl_models[[3]][[3]] %>% 
...
p
ggsave(p, filename=paste(figures, "greater_trachea_contrasts_by_weight.conservative_interactiontop.pdf", sep=""), width=4.5, height=3.5, device=cairo_pdf)
...
p
ggsave(p, filename=paste(figures, "greater_SDI_contrasts_by_weight.conservative_interaction.pdf", sep=""), width=4.5, height=3.5, device=cairo_pdf)
```
...
tidy_pred <- lesser_mtl_models[[3]][[3]] %>% 
  epred_draws(newdata = newdata , ndraws=50)
...
tidy_pred.f <- lesser_mtl_models[[3]][[3]] %>% 
  epred_draws(newdata = newdata.f )
...
tidy_pred.m <- lesser_mtl_models[[3]][[3]] %>% 
  epred_draws(newdata = newdata.m )
...
p
ggsave(p, filename=paste(figures, "lesser_raw_trachea_contrasts_by_weight.conservative.pdf", sep=""), width=4.5, height=3.5, device=cairo_pdf)
...
, xmax = 0.83078782, ymin = -0.02, ymax =  0.05,
  ...
  theme_cowplot()
p
ggsave(p, filename=paste(figures, "lesser_SDI_by_weight.conservative.pdf", sep=""), width=4.5, height=3.5, device=cairo_pdf)
...
p<-ggplot(SDI.plot, aes(x=x,y=Taxon, fill=Taxon))+
  geom_vline(xintercept = 0, lineype="dahsed", alpha=0.3)+
...
p
ggsave(p, filename = paste0(figures, "SDI_PP_trachea_grey.conservative.pdf"), width=5, height=4, device = cairo_pdf)
...
greater_mtl_models[[3]][[6]][[1]]
greater_mtl_models[[3]][[6]][[2]]
greater_mtl_models[[3]][[6]][[3]]
greater_mtl_models[[3]][[6]][[4]]
...
lesser_mtl_models[[3]][[6]][[1]]
lesser_mtl_models[[3]][[6]][[2]]
lesser_mtl_models[[3]][[6]][[3]]
lesser_mtl_models[[3]][[6]][[4]]
...
Here we plot conditional effects from best model for greaters
```{r, `conEffectGreater`, figures-side, fig.show="hold", out.width="50%"}
ce <- conditional_effects(greater_mtl_models[[3]][[4]])
...
```{r, `conEffectLesser`, figures-side, fig.show="hold", out.width="50%"}
#HERE WE USE the Interaction model just of exmine basically equivalent models
ce <- conditional_effects(lesser_mtl_models[[3]][[4]])
