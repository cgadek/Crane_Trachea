...
morpho_massive <-
  read_excel("data/Crane_harvest_data-WORKING_COPY.xls")
  ...
  mutate(
    ...
    sus = ifelse(
      taxon_name == "Antigone canadensis tabida" &
        ...
        sex == "M" &
        mean_trachea_length < 650 | taxon_name == "Antigone canadensis tabida" &
        sex == "F" &
        mass < 4000 |
        ...
        mass < 2500|
      taxon_name == "Antigone canadensis canadensis" &
        bursa == "Y" & mean_trachea_length <550,
      "sus",
      "chill"
    ),
    ...
    NK = if_else(is.na(NK), sample(40000:50000, n(), replace = FALSE), NK)
  ) %>%
...
cooks_dist <- function(dataframe, column){
  ...
  mod <- lm(dataframe[, 5] ~ 1, data = dataframe)
...
```
Only removes 3 points I can live with that!
...
dat.f%>%
  filter(taxon_name =="Antigone canadensis tabida",
         sex %in% c("M", "F"),,
         bursa %in% c("Y", "N", NA))%>%
ggplot(., aes(mass, mean_trachea_length, fill = bursa, size=sex))+
  geom_point( shape=21)
...
```{r}
write.csv(dat.f, "data/crane_conservative_full.csv")
